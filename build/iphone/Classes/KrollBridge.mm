...
@implementation DownloadMultipleFilesObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_downloadmultiplefiles);
	RELEASE_TO_NIL(modules);
...
-(void)injectPatches
{
	// called to inject any DownloadMultipleFiles patches in JS before a context is loaded... nice for 
...
{
	// create DownloadMultipleFiles global object
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_downloadmultiplefiles = [[DownloadMultipleFilesObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_downloadmultiplefiles];
	
	NSString *_downloadmultiplefilesNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _downloadmultiplefilesNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_downloadmultiplefiles valueForKey:name];
...
-(id)require:(KrollContext*)kroll path:(NSString*)path
{
	...
	@throw [NSException exceptionWithName:@"org.downloadmultiplefiles.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
